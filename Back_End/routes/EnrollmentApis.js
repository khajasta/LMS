...
router.route("/getallcoursesdata/:learnerid").get(async (req, resp) => {
    try {
        ...
        if (result == null) {
            ...
            console.log("🚀 ~ file: EnrollmentApis.js ~ line 290 ~ router.route ~ courses", courses)
            // mongoose query to get all the courses for the learner
            let courseresult = await Course.find({ _id: { $in: courses } });
            
            
            if (courseresult == null) {
                console.log("🚀 ~ file: EnrollmentApis.js ~ line 296 ~ router.route ~ courseresult", courseresult)
